...
are defined as a gradient boosting propensity score model.

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

# list of required packages
packages_needed <- c("dplyr", "ggplot2", "pbapply", "gbm")
...
cv_manual <- function(n, r, k_folds = 5) {
      ...
      # for loop for internal cross-validation
      for (i in 1:k_folds){
        ...
        # NUISANCE MODEL 1: gradient boosting propensity score model
        # develop propensity score model from train data and determine weights
        ps_null <- gbm(as.numeric(A)-1 ~ X1 + X2 + X3 + X4 + X5 + X6,
                   data = train,
                   distribution = "bernoulli",
                   n.trees = 100,         # number of boosting iterations
                   interaction.depth = 1,  # tree depth, 1 = small trees
                   shrinkage = 0.1,       # default learning rate
                   n.minobsinnode = 10    # minimum obs in terminal node / per leaf
                   )
        ...
        # this invovles estimatin NUISANCE MODEL 2 
        ...
        ps_model1 <- gbm(as.numeric(A)-1 ~ X1 + X2 + X3 + X4 + X5 + X6,
               data = df,
               distribution = "bernoulli",
               n.trees = 100,       
               interaction.depth = 1,  
        ...
        # Repeat steps 2-4 
        ps_model2 <- gbm(as.numeric(A)-1 ~ X1 + X2 + X3 + X4 + X5 + X6,
                   data = train,
                   distribution = "bernoulli",
                   n.trees = 100,         # number of boosting iterations
                   interaction.depth = 1,  # tree depth
                   shrinkage = 0.1,       # learning rate
                   n.minobsinnode = 10    # minimum obs in terminal node
                   )
        ...
        # STEP 1: train propensity score using training set
        # Repeat steps 2-4 
        ps_model3 <- gbm(as.numeric(A)-1 ~ X1 + X2 + X3 + X4 + X5 + X6,
                   data = test,
                   distribution = "bernoulli",
                   n.trees = 100,        
                   interaction.depth = 1,  
        ...
        ps_model4 <- gbm(as.numeric(A)-1 ~ X1 + X2 + X3 + X4 + X5 + X6,
                   data = nuisance,
                   distribution = "bernoulli",
                   n.trees = 100,         # number of boosting iterations
                   interaction.depth = 1,  # tree depth
...
for (n in sample_sizes) {
  ...
  results_cv_gbm <- do.call(rbind, results_cv_list)
  save(results_cv_gbm, file = paste0("results_GBM_", n, ".rda"))
...
loaded_results <- lapply(sample_sizes, function(n) {
  ...
  file_name <- paste0("results_GBM_", n, ".rda")
  data <- get(load(file_name))
  return(data)
})

# combine all datasets into one dataframe
combined_results_gbm <- do.call(rbind, loaded_results)
```
...
long_df0_gbm <- combined_results_gbm %>%
...
long_df0_gbm <- long_df0_gbm %>% mutate(strategy = recode(strategy,
                           "True_A0" = "True",
                           "Est1_A0" = "Full_Data",
...
boxplot0_gbm <- ggplot(long_df0_gbm, 
...
# ggsave("boxplot0_gbm.png", plot = boxplot0_gbm)
```
...
results_A0 <- combined_results_gbm %>% dplyr::select(replication, sample_size, 
...
```{r plot averages}
mean_briers0_gbm <- ggplot(data = summary_A0, 
...
# ggsave("mean_briers0_gbm.png", plot = mean_briers0_gbm)
...
```{r}
# transform into long format 
long_df1_gbm <- combined_results_gbm %>%
...
long_df1_gbm <- long_df1_gbm %>% mutate(strategy = recode(strategy,
                           "True_A1" = "True",
                           "Est1_A1" = "Full_Data",
...
# Boxplot: distribution of average estimates from 100 replications
boxplot1_gbm <- ggplot(long_df1_gbm, 
                      aes(x = strategy, y = brier_score, 
                          fill = strategy, color = strategy)) +
...
# ggsave("boxplot1_gbm.png", plot = boxplot1_gbm)
```


### Average Scores
```{r}
results_A1 <- combined_results_gbm %>% dplyr::select(replication, sample_size, 
...
```{r}
mean_briers1_gbm <- ggplot(data = summary_A1, 
       mapping = aes(x = as.factor(sample_size),
                     y = mean_brier, 
                     color = strategy, 
                     shape = strategy, 
  ...
  scale_y_continuous(limits = c(0.16, 0.28)) +
  
  theme_minimal()


# ggsave("mean_briers1_gbm.png", plot = mean_briers1_gbm)
